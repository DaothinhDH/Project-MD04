...
DELIMITER //
CREATE PROCEDURE PROC_FIND_PRODUCT_BY_ID_CATEGORY(IN new_id INT)
BEGIN
    SELECT *FROM product WHERE category_id = new_id;
END;
//
DELIMITER //
...
//
DELIMITER //
CREATE PROCEDURE PROC_UPDATE_STATUS_ORDER(IN o_id INT,IN o_status BIT)
BEGIN
    UPDATE orders SET order_status = o_status WHERE id = o_id;
end //;
